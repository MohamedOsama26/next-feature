...
class CreateFeatureCommand extends BaseCommand {
  ...
  void configure(ArgParser parser) {
    ...
    parser..addOption('feature-name', help: 'Name of the feature to create')
    ..addOption(
  ...
  void run(ArgResults results) {
    ...
    // Feature name
    String? featureName = results['feature-name'] as String? ?? (results.rest.isNotEmpty ? results.rest.first : null);
    ...
    if (results.wasParsed('state-management-type')) {
      ...
      stateManagementType = prompts.get('State management type (e.g., bloc, getx, provider, riverpod)', defaultsTo: 'bloc');
    }
    ...
    if (results.wasParsed('split-data-source')) {
      ...
      splitDataSource = prompts.getBool('Split data source into local and remote?', defaultsTo: false);
    }

    // Models
    List<String>? models = (results['models'] as List?)?.cast<String>();
    if (models == null || models.isEmpty) {
      String modelsInput = prompts.get('List of model names to create (comma separated, leave empty for none)');
      models = modelsInput.trim().isEmpty
          ? []
          : modelsInput.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList();
    }

    // Use cases
    List<String>? useCases = (results['use-cases'] as List?)?.cast<String>();
    if (useCases == null || useCases.isEmpty) {
      String useCasesInput = prompts.get('List of use case names to create (comma separated, leave empty for none)');
      useCases = useCasesInput.trim().isEmpty
          ? []
          : useCasesInput.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList();
    }

    // Screens
    List<String>? screens = (results['screens'] as List?)?.cast<String>();
    if (screens == null || screens.isEmpty) {
      String screensInput = prompts.get('List of screen names to create (comma separated, leave empty for none)');
      screens = screensInput.trim().isEmpty
          ? []
          : screensInput.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList();
    }
